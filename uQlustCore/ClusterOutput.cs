    ...
    public class ClusterOutput
    {
        ...
        static public ClusterOutput LoadExternal(string fileName,string dirName)
        {
            ...
            while (!r.EndOfStream)
            {
                ...
                if(!line.Contains("#") && line.Contains(":"))
                {
                    ...
                    aux.dirName = "F:\\casp10\\" + loc[loc.Length - 2];
        ...
        static public ClusterOutput LoadExternalPconsD(string fileName,string dirName)
        {
            ...
            while (!r.EndOfStream)
            {
                ...
                if (tmp.Length>0)
                {
                    KeyValuePair<string, double> auxK = new KeyValuePair<string, double>(tmp[0], Convert.ToDouble(tmp[1], System.Globalization.CultureInfo.InvariantCulture));
                    auxList.Add(auxK);
