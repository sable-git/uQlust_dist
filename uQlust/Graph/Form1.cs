...
namespace Graph
{
    ...
    public partial class AdvancedVersion : Form
    {
        string lastPath="";
...
/*            this.toolsToolStripMenuItem.DropDownItems.Remove(this.best5ToolStripMenuItem);
        ...
        private void SetDataDirOptions()
        {
            ...
            if (comboBox1.SelectedText.Length > 0)
            {
                ...
                set.mode = (INPUTMODE)Enum.Parse(typeof(INPUTMODE), comboBox1.SelectedText);
                set.Save();
            }
        ...
        {
            bool flag = CheckProfilesExist();
            KMeansForm km = new KMeansForm(manager.opt.kmeans, flag, (INPUTMODE)comboBox1.SelectedItem,GetAlignedProfiles());
        ...
        private void closeAdvanedUQlustToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if(parent!=null)
                parent.Show();
            exitProg = false;
        ...
        private void AdvancedVersion_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (parent!=null && exitProg)
                parent.Close();
        }
        ...
        private void uQlustTreeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            bool flag = CheckProfilesExist();

            uQlustTreeAdvanced uTree = new uQlustTreeAdvanced(manager.opt, ClusterAlgorithm.uQlustTree, (INPUTMODE)comboBox1.SelectedItem, flag, GetAlignedProfiles());
            DialogResult res= uTree.ShowDialog();
            if(res==DialogResult.OK)
            {
                manager.opt= uTree.localOpt;
                manager.opt.clusterAlgorithm.Clear();
                manager.opt.clusterAlgorithm.Add(uTree.alg);
        ...
        private void hierarchicalToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            bool flag = CheckProfilesExist();

            Aglomerative agl = new Aglomerative(manager.opt.hierarchical, ClusterAlgorithm.HierarchicalCluster, (INPUTMODE)comboBox1.SelectedItem, flag, GetAlignedProfiles());
            DialogResult res = agl.ShowDialog();
            if (res == DialogResult.OK)
            {
                manager.opt.hierarchical = agl.localOpt;
                manager.opt.clusterAlgorithm.Clear();
                manager.opt.clusterAlgorithm.Add(agl.alg);
