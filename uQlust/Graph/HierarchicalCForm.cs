        ...
        {            
            ...
            if(!radioButton3.Checked)
                if (distanceControl1.reference)
                {
                    ...
                    if(!distanceControl1.inputMode.Contains(mode))
                    {
                        MessageBox.Show("Profile "+distanceControl1.referenceProfile+" cannot be used in input mode"+mode);
                        this.DialogResult = DialogResult.None;
                        return;

                    }
                }
            if (distanceControl1.distDef == DistanceMeasures.HAMMING || distanceControl1.distDef == DistanceMeasures.COSINE)
            {
                ...
                if (distanceControl1.inputMode!=null && !distanceControl1.inputMode.Contains(mode))
                {
                    MessageBox.Show("Profile " + distanceControl1.referenceProfile + " cannot be used in input mode" + mode);
                    this.DialogResult = DialogResult.None;
                    return;
            ...
            if (FastHClusterRadio.Checked)
            {
                ...
                if (!jury1DSetup3.inputMode.Contains(mode))
                {
                    MessageBox.Show("Profile " + jury1DSetup3.profileName + " cannot be used in input mode" + mode);
                    this.DialogResult = DialogResult.None;
                    return;
            ...
            if(radioButton3.Checked)
            {
                ...
                if (userHash1.inputMode!=null && !userHash1.inputMode.Contains(mode))
                {
                    MessageBox.Show("Profile " + userHash1.profRefFile + " cannot be used in input mode" + mode);
                    this.DialogResult = DialogResult.None;
                    return;
